    ...
    <div className="space-y-6 p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm">
          <span className="text-base font-bold text-slate-400 uppercase tracking-wider block mb-1">Raw Pipeline</span>
          <span className="text-xl font-extrabold text-slate-800 dark:text-slate-100 tabular-nums">
            {currency(totalRaw)}
          </span>
          <span className="text-base text-slate-400 block mt-1">Sum of all active deals</span>
        </div>

        <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm">
          <span className="text-base font-bold text-slate-400 uppercase tracking-wider block mb-1">Weighted Pipeline</span>
          <span className="text-xl font-extrabold text-emerald-600 dark:text-emerald-450 tabular-nums">
            {currency(totalWeighted)}
          </span>
          <span className="text-base text-slate-400 block mt-1">Adjusted for stage probability</span>
        </div>

        <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm">
          <span className="text-base font-bold text-slate-400 uppercase tracking-wider block mb-1">Forecast Variance</span>
          ...
          <span className="text-base text-slate-400 block mt-1">Actual Closed vs. Target</span>
        </div>

        <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm">
          <span className="text-base font-bold text-slate-400 uppercase tracking-wider block mb-1">Forecast Accuracy</span>
          ...
          <span className="text-base text-slate-400 block mt-1">Goal: &gt;90% accuracy</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-5 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {stages.map((stage, idx) => (
              <div key={stage.name} className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center text-base">
                <div className="sm:col-span-4 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-base">
                    {idx + 1}
                  </span>
            ...
            <div className="space-y-3 text-base">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-base rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              {stages.map((stage) => {
                ...
                return (
                  <div key={stage.name} className="flex flex-col items-center group w-7">
                    {/* Tooltip value */}
                    <span className="text-base font-bold text-slate-500 dark:text-slate-400 mb-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    ...
                    />
                    <span className="text-base font-semibold text-slate-400 mt-2 truncate w-7 text-center">
      ...
      <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-5 shadow-sm space-y-4">
        ...
        <p className="text-base text-slate-500 dark:text-slate-400">Adjust monthly targets and actuals to see trailing accuracy trend.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {history.map((month, idx) => (
            <div key={idx} className="space-y-2 p-3 rounded-md border border-slate-100 dark:border-slate-800">
              <span className="text-base font-semibold text-slate-600 dark:text-slate-400">Month {idx + 1}</span>
              <div className="space-y-1">
                <label className="text-base text-slate-400 uppercase font-medium">Target</label>
                <input
                  type="number"
                  value={month.target}
                  onChange={(e) => handleHistoryChange(idx, 'target', parseInt(e.target.value))}
                  className="w-full px-2 py-1 text-base rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700"
                />
              </div>
              <div className="space-y-1">
                <label className="text-base text-slate-400 uppercase font-medium">Actual</label>
                <input
                  type="number"
                  value={month.actual}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
          <span className="text-base text-slate-500">Average Accuracy (trailing 3M)</span>
      ...
      <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-5 shadow-sm space-y-4">
        ...
        <p className="text-base text-slate-500 dark:text-slate-400">Simulate the effect of average deal discount on your expected weighted pipeline revenue.</p>

        <div className="flex items-center gap-4">
          <div className="flex-1 space-y-2">
            <label className="text-base font-bold text-slate-400 uppercase tracking-wider">Discount Rate</label>
            <input
              type="range"
              min="0"
              max="30"
          ...
          <div className="text-center">
            ...
            <span className="text-base text-slate-400 uppercase font-bold">Applied</span>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4 pt-2">
          <div>
            <span className="text-base text-slate-400 block">Weighted Pipeline (No Discount)</span>
            <span className="text-base font-bold text-slate-800 dark:text-slate-200 tabular-nums">{currency(totalWeighted)}</span>
          </div>
          <div>
            <span className="text-base text-slate-400 block">After Discount</span>
