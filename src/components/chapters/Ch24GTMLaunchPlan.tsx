  ...
  return (
    <div className="space-y-6 p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-3 shadow-sm">
        <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pl-2">
        ...
        <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-5 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-3 text-xs">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">TAM (Total Addressable Market)</label>
                <input
                  type="number"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">SAM (Serviceable Addressable)</label>
                <input
                  type="number"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">SOM (Serviceable Obtainable)</label>
                <input
                  type="number"
            ...
            <div className="flex items-center justify-center">
              <svg width="150" height="150" viewBox="0 0 150 150">
                ...
                <text x="75" y="75" textAnchor="middle" fill="currentColor" className="text-[9px] font-extrabold text-emerald-650 dark:text-emerald-450 pointer-events-none select-none">SOM</text>
              </svg>
            </div>
          </div>
        </div>
          ...
          {mode === 'top-down' ? (
            <>
              ...
              <div className="space-y-3">
                {channels.map((chan, idx) => (
                  <div key={chan.channelName} className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center text-xs">
              ...
              <div className="pt-2 space-y-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Conversion Rate per Channel</span>
                {channels.map((chan, idx) => (
                  <div key={chan.channelName} className="grid grid-cols-3 gap-2 items-center text-xs">
                ...
                <div className="flex justify-between text-xs font-bold pt-1 border-t border-slate-100 dark:border-slate-800">
            ...
            <>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="flex justify-between items-center text-xs font-bold pt-4 border-t border-slate-105 dark:border-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800/80 p-5 shadow-sm space-y-4">
          ...
          <div className="space-y-4 max-h-[25vh] overflow-y-auto pr-1">
            {phases.map((ph, idx) => (
              <div key={ph.phase} className="flex gap-3 text-xs relative">
                {/* Visual node line */}
                {idx < phases.length - 1 && (
                  <div className="absolute top-5 bottom-0 left-2.5 w-[2px] bg-slate-200 dark:bg-slate-800" />
                )}
                {/* Node circle */}
                <div className={`w-5 h-5 rounded-full shrink-0 flex items-center justify-center font-bold text-[9px] z-10 ${
                  ph.status === 'Completed'
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex justify-between items-center font-bold">
                    ...
                    <select
                      ...
                      className="text-[10px] px-1 py-0.5 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-650"
                    >
                  ...
                  <input
                    ...
                    className="w-full text-xs bg-transparent border-b border-slate-150 dark:border-slate-800 focus:outline-none focus:border-emerald-500 text-slate-600 dark:text-slate-400 pb-0.5"
                  />
                  {/* NEW: Confidence slider per phase */}
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-slate-500 w-20">Confidence</span>
                    <input
                      type="range"
                      min="0"
                      max="100"
                    ...
                    />
                    <span className="text-[10px] font-bold text-slate-650 w-8 text-right">{ph.confidence || 50}%</span>
                  </div>
                </div>
              </div>
            ))}
